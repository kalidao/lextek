    ...
    function sections(DEAL calldata deal) public pure returns (string memory) {
        return string(
            abi.encodePacked(
                ...
                deal.expiryDate,
                '. Payment shall be rendered as of the Expiry Date for the Work, or if pending, at the option of the Client through the `unlock()` method after their `escrow()` into the protocol addressed onchain at 0x000000000000275b61D4CE184F15B495014B1098 (with the ID of this Deal hashed thereby, the "ESCROW"). The Client shall be entitled to `reclaim()` their assets from the ESCROW after the Expiry Date if undisputed (see below, Miscellaneous, (c) Onchain Arbitration).</p>',
